...
function onNewState(adapter, { channel, balances, newMsg, approveMsg }) {
	...
	if (!isValidTransition(channel, prevBalances, newBalances)) {
		console.error(
			`validatatorWorker: ${channel.id}: invalid transition requested in NewState`,
			prevBalances,
			newBalances
		)
	...
	if (!isValidValidatorFees(channel, newBalances, balancesAfterFees)) {
		console.error(
			`validatatorWorker: ${channel.id}: invalid validator fees requested in NewState`,
			toBNStringMap(newBalances),
			toBNStringMap(balancesAfterFees)
		)
		return { nothingNew: true }
	}
	...
	return adapter.verify(leader.id, stateRoot, signature).then(function(isValidSig) {
		if (!isValidSig) {
			console.error(
				`validatatorWorker: ${channel.id}: invalid signature NewState`,
				prevBalances,
				newBalances
			)
			return { nothingNew: true }
